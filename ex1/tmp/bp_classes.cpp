...
SIM_stats Btb::getStats() { return stats; }

void Btb::handleJump(uint32_t pc)
{
	check if entry exists - if so, handle it by updating history;
	else enter entry into entries
}
