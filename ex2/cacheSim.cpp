...
int main()
{
	uint32_t address = 0xFA54DDAA;
	int offsetSize = 4;
	int setSize = 4;
	int tagSize = 32 - offsetSize - tagSize;
	Address add = Address(address, offsetSize, setSize, tagSize);
	//add.print();

	Entry entry = Entry(address, offsetSize, setSize, tagSize, 0);
	entry.print();

	unsigned cacheSize = 5;
	unsigned blockSize = 4;
	int assoc = FULLY_ASSOCIATIVE;
	Cache c = Cache(cacheSize, blockSize, assoc);
	cout << endl << c.entries.size();
	c.entries[0] = entry;
	Entry& e = c.findEntryByAddress(address);
	e.print();
}
